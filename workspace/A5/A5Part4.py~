...
def selectFlatPhasePeak(pX, p, phaseDevThres):
    ...
    nBinLow = max(p - int(np.floor(nstdSamples/2)),0)
    nBinHigh = min(p + int(np.floor(nstdSamples/2)) +1,len(pX))

    pXstd = pX[nBinLow:nBinHigh]
    rstd = np.std(pXstd)

    selectFlag = (rstd < phaseDevThres)
#    print 'selectFlag =' +str(selectFlag)
    
    return bool(selectFlag)
...
z = selectFlatPhasePeak(pX, p, phaseDevThres)
...
#phaseDevThres = 0.01
#p = 17
#pX = np.array([2.39,2.40,2.40,2.41,3.37,2.45,2.46,2.46,2.29,1.85,2.34,2.18,2.93,2.10,3.39,2.41,2.41,2.40,2.40,2.40,1.46,0.23,0.98,0.41,0.37,0.40,0.41,0.87,0.51,0.67]) 

#(tStamps, tfreq) = sineModelAnalEnhanced()
