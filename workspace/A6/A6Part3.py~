...
def estimateInharmonicity(inputFile = '../../sounds/piano.wav', t1=0.1, t2=0.5, window='hamming', 
    ...
    frameLow = int(np.ceil(t1*float(fs)/H))
    frameHigh = int(np.floor(t2*float(fs)/H))


    inharmonicity_M = np.array([])
    for j in range(frameLow,frameHigh + 1):
        inharmonicity_j = 0.0
        R = 0.0 
        for r in range(0, nH):
            if(xhfreq[j,r]>0.0):
                inharmonicity_j = inharmonicity_j + np.abs(xhfreq[j,r] - (r+1)*xhfreq[j,0])/float(r+1)
                R = R + 1.0
        inharmonicity_M = np.append(inharmonicity_M,inharmonicity_j/max(R,1.0))

    inharmonicity = sum(inharmonicity_M)/float(frameHigh - frameLow + 1)
...
#print 'frameLow = ' + str(frameLow)
#print 'frameHigh = ' + str(frameHigh)
