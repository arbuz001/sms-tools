...
def optimalZeropad(x, fs, f):
    ...
    """
    M = len(x)
    nSamplesInPeriod = fs/f

    N = math.ceil(float(M)/nSamplesInPeriod)
    N = int(N*nSamplesInPeriod)

    nToAdd = N - M

    x = np.append(x,np.zeros([nToAdd]))
    mx = fft(x)

    return 20*np.log10(abs(mx[0:(N/2)+1]))
