...
def extractMainLobe(window, M):
    ...
    idx_max_mX = np.where(mX == max(mX))[0][0]

    idx = idx_max_mX
    bContinue = True
    while ((idx<=N) and (idx >=0) and bContinue):
        bContinue = ( mX[idx+1] <= mX[idx])
        if(not(bContinue)):
            break 
        idx += 1
    ...
    mXmlobe = mX[nLowerLobe:nUpperLobe+1]
...
# mXmlobe = extractMainLobe(window, M)
